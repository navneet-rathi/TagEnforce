...
#    AllInstances= ec2.instances.all()  # all instances in acc
    AllInstances= ec2.instances.filter(Filters=AllRunning)
#   ----------------------------------------------------------------------------------------     
    ...
    ListRunningUnTaggedInstances = list((set(ListAllindtance) - set(ListAllRunningTInstances)))
    #
    # Code is modified
...
# Uncomment this line only after confirmation to stop instances.           
...
def publish_to_sns(message):
    ...
    response = sns.publish(TopicArn=Topic_Arn,Subject = "Instance Shutdown Notification", Message = sns_message)